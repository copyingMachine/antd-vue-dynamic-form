        ...
        <template #item="{ element, index }">
          <transition-group name="fade" tag="div">
            <template v-if="element.type === 'grid'">
              <a-row class="widget-col widget-view" type="flex" v-if="element.key" :key="element.key"
                ...
                <a-col v-for="(col, colIndex) of element.columns" :key="colIndex" :span="col.span ?? 0">
                  <Draggable class="widget-col-list" item-key="key" ghostClass="ghost" handle=".drag-widget"
                    ...
                    <template #item="{ element, index }">
                      <transition-group name="fade" tag="div">
                        <AntdWidgetFormItem v-if="element.key" :key="element.key" :element="element"
                ...
                <div class="widget-view-action widget-col-action" v-if="widgetFormSelect?.key === element.key">
                  <DeleteOutlined iconClass="delete" @click.stop="handleDeleteClick(index, widgetForm.list)" />
                </div>

                <div class="widget-view-drag widget-col-drag" v-if="widgetFormSelect?.key === element.key">
                  <DragOutlined iconClass="move" className="drag-widget" />
                </div>
              </a-row>
            </template>
            <template v-else>
